...
def get_storage_image():
    img_boat1 = Image.open(os.path.join(myCWD,"images","vessel_layout_test1.jpeg")) # 788 x 1976
    ...
    if vessel_inventory['tools'][get_tool_info][1] == 'starboard':
        ...
        storage_selection = img_boat1.crop(other_storage)
...
with open(os.path.join(myCWD,'toolBox',toolFile), 'r') as f:
	data = f.read()
...
def add_new_tool():
    ...
    if addingTOOL == 'y':
        while 1:
            ...
            with io.open(os.path.join(myCWD,'toolBox',toolFile), 'w', encoding='utf8') as outfile:
